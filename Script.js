...
function updatePotInfo() {
  ...
  potSizeEl.textContent = pot.toFixed(0);
  toCallEl.textContent = toCall.toFixed(0);
  stageLabelEl.textContent = STAGES[currentStageIndex].toUpperCase();
  scenarioLabelEl.textContent = scenario ? scenario.label : "—";
}
...
function maybeStartTimer() {
  if (difficulty === "beginner") {
    clearTimer();
    timerCountdownEl.textContent = "No timer in Beginner Mode";
  } else {
    startTimer();
  }
}
...
function analyzeBoard(board, hole) {
  // longest consecutive run on BOARD ONLY
  ...
  if (isFlop) {
    // Flop: 3 in a row => Connected, 1-gap shapes => Semi-Connected
    ...
    // Turn/River: 4-to-straight and straight on board checks (BOARD ONLY)
    const rv = new Set(board.map(c => RANK_TO_VAL[c.rank]));
    if (rv.has(14)) rv.add(1);
    let ordered = [...rv].sort((a,b)=>a-b);
    // straight on board (river)
    let run=1, maxRun=1;
...
function updateHintsImmediate() {
  ...
  // Optional prose summary line (uncomment to show a sentence)
  // const proseFlags = [];
  // if (texture.paired) proseFlags.push("paired (boat risk)");
  // if (texture.mono) proseFlags.push("flush possible");
  // if (texture.fourToStraight || texture.connected) proseFlags.push("straight possible");
  // const proseLine = proseFlags.length ? `<div style="opacity:.85">Board is ${proseFlags.join(", ")}.</div>` : "";
  ...
  const html = `
    <div style="display:flex;flex-direction:column;gap:6px">
      ...
      <div><strong>Made hand:</strong> ${made.label} ${nutsBadge}</div>
      <div><strong>Outs:</strong><br/>${outsLines.join("")}</div>
      ${potLine}
      <!-- ${'proseLine placeholder'} -->
    </div>
  `;
  ...
  if (difficulty === "beginner") {
    ...
    hintsEl.innerHTML = `
      <div><strong>Board:</strong> ${boardTags || '<span class="badge green">Stable</span>'}</div>
      <div><strong>Made hand:</strong> ${made.label} ${nutsBadge}</div>
      <div><strong>Outs:</strong> ${outsInfo.strong} strong${outsInfo.tentative?`, ${outsInfo.tentative} tentative`:''}</div>
    `;
  } else {
    hintsEl.innerHTML = ""; // expert: silent
  }
}
...
const SETTINGS = {
  sim: {
    ...
    rangeAwareContinuation: true,  // ON by default
...
function villainConnected(hole, boardAtStreet) {
  ...
  const pairWithBoard = byRankBoard[hole[0].rank] > 0 || byRankBoard[hole[1].rank] > 0 || (hole[0].rank === hole[1].rank);
...
function downloadCsv() {
  ...
  const headers = ["stage","equityInput","equityActual","potOddsInput","potOddsActual","decision","equityBand","potOddsBand","decisionBand","timeUsed"];
  const rows = [headers.join(",")];
  sessionHistory.forEach((h) => {
    const row = [
      h.stage,
      h.equityInput.toFixed(2),
      h.equityActual.toFixed(2),
      h.potOddsInput.toFixed(2),
      h.potOddsActual.toFixed(2),
      h.decision,
      h.equityBand,
      h.potOddsBand,
      h.decisionBand,
      h.timeUsed == null ? "" : h.timeUsed.toFixed(2)
    ];
    rows.push(row.join(","));
  });
  ...
  a.href = url; a.download = "poker_equity_trainer_history.csv"; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
}
...
difficultySelect.addEventListener("change", () => {
  ...
  if (difficulty === "beginner") {
    timerRange.disabled = true; clearTimer(); timerCountdownEl.textContent = "No timer in Beginner Mode";
  } else {
